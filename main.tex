...
\ctparttext{The scaling of indicators with the population size of cities has been
recently re-discovered and explored by the different communities.%SHITE!!!! 

The contribution of this part is threefold. First, we review the exisiting
literature, pre- and post-2007 on allometric scalings, sorting the measured
exponents by theme. We then propose a model to explain the scaling of several
...
challenges posed by these scaling relationships, namely their interpretation,
...
\ctparttext{Residential segregation is a reality. A reality so rife that it has
    ...
    a measure of attraction of the different categories, which allows us to
    define unambiguously classes from the original categories. We also study the
properties of neighbourhoods of different classes, and revisit the traditional poor
...
\ctparttext{People, energy, information and goods are carried through cities and
    systems of cities thanks to various networks. In this part, we succintly
    present our work on these spatial networks. We first propose a quantative
    method to classify cities based on a new perspective on street patterns and
    the use of OpenStreetMaps' database. In the second chapter, we propose an
    model for the growth of spatial networks based on cost-benefit analysis. The
    resulting networks exhibit a crossover between the star graph and the
    minimum spanning tree when the ratio of costs and benefits evolve. In the
    intermediate regime, the networks adopt a hub-and-spoke hierarchical
    structure that has many interesting properties. Finally, we conclude this part with a
    large-scale description of subway and railway networks. Using the model
    presented in the previous chapter, we are able to predict many of their
    properties based on the characteristics of the underlying city or country.}
