...
\begin{equation} 
    \boxed{k \sim \left( \frac{\overline{P}}{P^*} \right)^{\frac{\mu}{\mu
    + 1}}}
...
topic~\cite{Glaser:2008, Brueckner:1999}, we realised that there was very little
empirical knowledge on segregation that could be used to test a model. This led
us to the study that we present in the third part of this thesis.
...
is that they do not consider the spatial contiguity of different areal units. It
might be that we find different areal nits that are in fact part of the same
center [Show example in the US].
The number they provide probably does not reflect the `real' number of centers
(if there is such a thing). But somehow, assuming that different cities exhibit
similar structures, they should still provide values that are coherent across
different urban areas, and are usefull for comparison purposes.  
